...
const Tooltip: React.FC<TooltipProps> = ({ children, text, position = "top" }) => {
  return (
    <div className="relative group">
      {children}
      <div className={`absolute z-50 px-3 py-2 text-sm font-medium text-white bg-gray-900 rounded-lg shadow-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap
...
export default function HomePage() {
  ...
  const handleContribute = async () => {
    ...
    try {
      const amountInMicroSTX = Math.floor(parseFloat(contributionAmount) * 1_000_000)
      const senderAddress = user?.profile?.stxAddress?.testnet || user?.profile?.stxAddress?.mainnet
      const postConditions = [
        makeStandardSTXPostCondition(
          senderAddress,
          FungibleConditionCode.LessEqual,
          BigInt(amountInMicroSTX)
        )
      ]
        ...
        <div className="container mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-2xl font-bold text-white">CrowdStacks</span>
            </div>
            <div className="flex items-center space-x-6">
              ...
              <Link href="/create" className="flex items-center space-x-2 text-white/70 hover:text-blue-400 transition-colors">
                <Plus size={20} />
                <span>Create</span>
              </Link>
              <Link href="/admin" className="flex items-center space-x-2 text-white/70 hover:text-blue-400 transition-colors">
                <BarChart3 size={20} />
                <span>Admin</span>
              </Link>
      ...
      <main className="container mx-auto px-6 py-12">
        ...
        <section className="text-center mb-16">
          ...
          <p className="text-xl text-gray-300 max-w-2xl mx-auto">
            Fund the future on Bitcoin with transparent, decentralized crowdfunding
          </p>
          {loading && (
            <div className="mt-4 text-blue-400">Loading blockchain data...</div>
          )}
        </section>
          ...
          <section className="mb-8">
            <div className="flex justify-center">
              <select
                ...
                className="bg-gray-800 text-white px-4 py-2 rounded-lg border border-gray-600 focus:border-blue-400"
              >
        ...
        {goalReached && currentCampaign.total > 0 && (
          <section className="mb-12">
            <div className="backdrop-blur-md bg-green-500/20 border border-green-400/30 rounded-2xl p-6 text-center">
            ...
            <div className="grid lg:grid-cols-3 gap-8 items-start">
              ...
              <div className="lg:col-span-2">
                <div className="flex items-center space-x-2 mb-4">
                  <h2 className="text-3xl font-bold text-white">{currentCampaign.title}</h2>
                  {goalReached && <span className="text-2xl">🏆</span>}
                </div>
                <p className="text-gray-300 mb-2">{currentCampaign.description}</p>
                <p className="text-sm text-gray-400 mb-6">Owner: {currentCampaign.owner}</p>
                  ...
                  <div className="grid grid-cols-3 gap-4 text-center">
                    <div>
                      <div className="flex items-center justify-center space-x-2 text-blue-400">
                        <Target size={20} />
                        <span className="text-2xl font-bold text-white">{currentCampaign.total.toFixed(1)}</span>
                      </div>
                      <p className="text-gray-400 text-sm">STX Raised</p>
                    </div>
                    
                    <div>
                      <div className="flex items-center justify-center space-x-2 text-purple-400">
                        <Target size={20} />
                        <span className="text-2xl font-bold text-white">{currentCampaign.goal.toFixed(1)}</span>
                      </div>
                      <p className="text-gray-400 text-sm">STX Goal</p>
                    </div>
                    
                    <div>
                      <div className="flex items-center justify-center space-x-2 text-green-400">
                        <Calendar size={20} />
                        <span className="text-2xl font-bold text-white">{currentCampaign.active ? 'Active' : 'Closed'}</span>
                      </div>
              ...
              <div className="space-y-4">
                <div className="backdrop-blur-sm bg-white/5 rounded-xl p-6 border border-white/10">
                  ...
                  <div className="space-y-4">
                    <div>
                      <label className="block text-gray-300 text-sm mb-2">Amount (STX)</label>
                      <input
                        type="number"
                        ...
                        min="1"
                        step="0.1"
                        className="w-full bg-gray-800/50 border border-gray-600 rounded-lg px-4 py-3 text-white placeholder-gray-400 focus:outline-none focus:border-blue-400 transition-colors"
                    ...
                    <button
                      ...
                      className="w-full bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 disabled:from-gray-600 disabled:to-gray-600 text-white font-semibold py-3 px-6 rounded-lg flex items-center justify-center space-x-2 transition-all duration-200 disabled:cursor-not-allowed"
                    >
                      <Wallet size={20} />
                    ...
                    {!user && (
                      <p className="text-xs text-yellow-400 text-center">Connect wallet to contribute</p>
                    )}
                  </div>
                </div>
              </div>
        ...
        <section className="mb-16">
          <div className="grid md:grid-cols-3 gap-6">
            <div className="backdrop-blur-md bg-white/10 rounded-xl p-6 border border-white/20 text-center">
              ...
              <h3 className="text-3xl font-bold text-white mb-2">{globalStats.totalRaised.toFixed(1)}</h3>
              <p className="text-gray-400">Total STX Raised</p>
            </div>
            
            <div className="backdrop-blur-md bg-white/10 rounded-xl p-6 border border-white/20 text-center">
              <Users className="h-8 w-8 text-purple-400 mx-auto mb-4" />
              <h3 className="text-3xl font-bold text-white mb-2">{globalStats.totalContributors}</h3>
              <p className="text-gray-400">Total Contributors</p>
            </div>
            
            <div className="backdrop-blur-md bg-white/10 rounded-xl p-6 border border-white/20 text-center">
              <BarChart3 className="h-8 w-8 text-green-400 mx-auto mb-4" />
              <h3 className="text-3xl font-bold text-white mb-2">{globalStats.activeCampaigns}</h3>
        ...
        {campaigns.length > 0 && (
          <section className="mb-16">
            <h2 className="text-3xl font-bold text-white text-center mb-8">All Campaigns</h2>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {campaigns.map((campaign, index) => {
                ...
                return (
                  <div
                    key={index}
                    className={`backdrop-blur-md bg-white/10 rounded-xl p-6 border border-white/20 cursor-pointer transition-all hover:bg-white/20 ${
                      selectedCampaign === index ? 'ring-2 ring-blue-400' : ''
                    }`}
                  ...
                  >
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between text-sm">
                        <span className="text-gray-400">Progress</span>
                        <span className="text-white">{campaignProgress.toFixed(1)}%</span>
                      </div>
                      ...
                      <div className="flex justify-between text-xs text-gray-400">
                        <span>{campaign.total.toFixed(1)} STX</span>
                        <span>{campaign.goal.toFixed(1)} STX</span>
                      </div>
                      
                      <div className="flex justify-between items-center">
                        <span className={`text-xs px-2 py-1 rounded ${
                          campaign.active ? 'bg-green-600 text-white' : 'bg-gray-600 text-gray-300'
                        }`}>
        ...
        <section>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <div className="text-center">
              ...
              <h3 className="text-xl font-semibold text-white mb-4">1. Create Campaign</h3>
              <p className="text-gray-300">Set your funding goal and campaign details on the Stacks blockchain</p>
            </div>
            ...
            <div className="text-center">
              ...
              <h3 className="text-xl font-semibold text-white mb-4">2. Get Funded</h3>
              <p className="text-gray-300">Share your campaign and receive STX contributions from supporters</p>
            </div>
            ...
            <div className="text-center">
              ...
              <h3 className="text-xl font-semibold text-white mb-4">3. Build & Deliver</h3>
              <p className="text-gray-300">Use the funds to build your project and deliver value to Bitcoin ecosystem</p>
            </div>
          </div>
        </section>
      </main>
